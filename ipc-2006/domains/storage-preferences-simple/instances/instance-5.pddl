...
(:goal (and
	...
	(forall (?c - crate)
	   (preference p12A (exists (?d - depot) (and (in ?c ?d) (not (= ?d depot3))))))
	...
	(forall (?c1 ?c2 - crate ?d - depot)
	   (preference p2A (imply (and (in ?c1 ?d) (in ?c2 ?d) (not (= ?c1 ?c2)))
			          (compatible ?c1 ?c2))))

	(forall (?c1 ?c2 - crate ?s1 ?s2 - storearea)
	   (preference p7A (imply (and (on ?c1 ?s1) (on ?c2 ?s2)
			               (not (= ?c1 ?c2)) (connected ?s1 ?s2))
			          (compatible ?c1 ?c2))))

	(forall (?d - depot ?s - storearea ?h - hoist)
	   (preference p3E (imply (and (at ?h ?s) (in ?s ?d))
