...
(:goal (and
	...
	(forall (?c - crate)
	   (preference p14A (exists (?d - depot) (and (in ?c ?d) (not (= ?d depot3))))))
	(forall (?c - crate)
	   (preference p18A (exists (?d - depot) (and (in ?c ?d) (not (= ?d depot4))))))
	(forall (?c - crate)
	...
	(forall (?h - hoist)
	   (preference p14B (exists (?s - storearea ?d - depot)
			      (and (at ?h ?s) (in ?s ?d) (not (= ?d depot3))))))
	(forall (?h - hoist)
	   (preference p18B (exists (?s - storearea ?d - depot)
	...
	(forall (?c1 ?c2 - crate ?d - depot)
	   (preference p7A (imply (and (in ?c1 ?d) (in ?c2 ?d) (not (= ?c1 ?c2)))
			          (compatible ?c1 ?c2))))

	(forall (?c1 ?c2 - crate ?s1 ?s2 - storearea)
	   (preference p18C (imply (and (on ?c1 ?s1) (on ?c2 ?s2)
			               (not (= ?c1 ?c2)) (connected ?s1 ?s2))
			          (compatible ?c1 ?c2))))

	(forall (?d - depot ?s - storearea ?h - hoist)
	   (preference p35A (imply (and (at ?h ?s) (in ?s ?d))
