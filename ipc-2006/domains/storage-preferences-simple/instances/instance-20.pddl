...
(:goal (and
	...
	(forall (?c - crate)
	   (preference p34A (exists (?d - depot) (and (in ?c ?d) (not (= ?d depot7))))))
	(forall (?c - crate)
	   (preference p39A (exists (?d - depot) (and (in ?c ?d) (not (= ?d depot8))))))
	(forall (?c - crate)
	   (preference p44A (exists (?d - depot) (and (in ?c ?d) (not (= ?d depot9))))))
	(forall (?c - crate)
	   (preference p48A (exists (?d - depot) (and (in ?c ?d) (not (= ?d depotA))))))

	(forall (?h - hoist)
	   (preference p6B (exists (?s - storearea ?d - depot)
			      (and (at ?h ?s) (in ?s ?d) (not (= ?d depot1))))))
	(forall (?h - hoist)
	   (preference p11B (exists (?s - storearea ?d - depot)
	...
	(forall (?h - hoist)
	   (preference p34B (exists (?s - storearea ?d - depot)
			      (and (at ?h ?s) (in ?s ?d) (not (= ?d depot7))))))
	(forall (?h - hoist)
	   (preference p39B (exists (?s - storearea ?d - depot)
			      (and (at ?h ?s) (in ?s ?d) (not (= ?d depot8))))))
	(forall (?h - hoist)
	   (preference p44B (exists (?s - storearea ?d - depot)
			      (and (at ?h ?s) (in ?s ?d) (not (= ?d depot9))))))
	(forall (?h - hoist)
	   (preference p48B (exists (?s - storearea ?d - depot)
	...
	(forall (?c1 ?c2 - crate ?s1 ?s2 - storearea)
	   (preference p25C (imply (and (on ?c1 ?s1) (on ?c2 ?s2)
			               (not (= ?c1 ?c2)) (connected ?s1 ?s2))
			          (compatible ?c1 ?c2))))

	(forall (?d - depot ?s - storearea ?h - hoist)
	   (preference p49A (imply (and (at ?h ?s) (in ?s ?d))
