...
(:goal (and
	...
	(forall (?l - level)
	   (preference p5A (imply (stored goods13 ?l) (stored goods14 ?l))))

	(preference p5B (imply (stored goods13 level1) (stored goods14 level2)))
	(preference p5C (imply (stored goods13 level2) (stored goods14 level4)))
	(preference p5D (not (stored goods13 level3)))
	(preference p5E (not (stored goods13 level4)))
	(preference p5F (not (stored goods13 level5)))

	(preference p5G (imply (stored goods8 level1) (stored goods10 level2)))
	(preference p5H (imply (stored goods8 level2) (stored goods10 level4)))
	(preference p5I (not (stored goods8 level3)))
	(preference p5J (not (stored goods8 level4)))
	(preference p5K (not (stored goods8 level5)))

	(forall (?l - level)
	   (preference p5L (imply (stored goods13 ?l) (stored goods14 ?l))))

	(forall (?l - level)
	   (preference p5M (imply (stored goods5 ?l) (stored goods13 ?l))))
