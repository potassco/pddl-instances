...
(define (problem roverprob17923-4)
        ...
        (:goal
         (and (preference g0 (communicated_soil_data waypoint7))
              ...
              (preference g6 (communicated_image_data objective7 low_res))
              (preference g7 (communicated_image_data objective5 low_res))
              (preference g8 (communicated_image_data objective4 low_res))))
        (:metric minimize
         (+ (* (is-violated g8) 49)
            (* (is-violated g7) 45.1)
            (* (is-violated g6) 52.3)
            (* (is-violated g5) 133.9)
            (* (is-violated g4) 193.9)
            (* (is-violated g3) 144.4)
            (* (is-violated g2) 141.9)
            (* (is-violated g1) 84.5)
            (* (is-violated g0) 253.3)
            (sum-traverse-cost))))
