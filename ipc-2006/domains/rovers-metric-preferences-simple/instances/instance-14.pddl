...
(define (problem roverprob19841-5)
        ...
        (:goal
         (and (preference g0 (communicated_soil_data waypoint14))
              (preference g1
               (and (communicated_soil_data waypoint11)
                    (communicated_soil_data waypoint14)))
              (preference g2
               (and (communicated_soil_data waypoint7)
                    (communicated_soil_data waypoint14)))
              (preference g3 (communicated_soil_data waypoint11))
              (preference g4 (communicated_soil_data waypoint7))
              (preference g5 (communicated_soil_data waypoint4))
              (preference g6 (communicated_soil_data waypoint3))))
        (:metric minimize
         (+ (* (is-violated g6) 180.9)
            (* (is-violated g5) 9.2)
            (* (is-violated g4) 40.2)
            (* (is-violated g3) 71.1)
            (* (is-violated g2) 162.8)
            (* (is-violated g1) 207.5)
            (* (is-violated g0) 60.4)
            (sum-traverse-cost))))
