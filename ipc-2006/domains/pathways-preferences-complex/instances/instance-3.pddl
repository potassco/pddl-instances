...
(:goal
	(and
	(preference p0A (>= (+ (available pRbp1-E2F4-DP12p1) (available SP1-p107-gP)) 2))
	(preference p0B (>= (+ (available cycA) (available SP1-p107p1-gP)) 6))
	(preference p0C (>= (+ (available cdk46p1-cycD) (available HDAC1-p107-E2F4-DP12p1)) 6))))


(:constraints (and

	(preference p1A (sometime-before (> (available SP1-p107p1-gP) 0) (> (available p107p1) 0)))
	(preference p1B (sometime-before (> (available cdk46p1-cycD) 0) (> (available SP1-gP) 0)))

	(preference p1C (always (= (available p107) 0)))

	(preference p1D (sometime (> (available cycE) 0)))
	(preference p1E (sometime-after (> (available cycE) 0) (> (available p107-E2F4-DP12p1) 0)))))

(:metric minimize (+ (* 3.5 (is-violated p0A))
		     (* 2.5 (is-violated p0B))
		     (* 3.0 (is-violated p0C))
		     (* 1 (is-violated p1A))
		     (* 1 (is-violated p1B))
