...
(:goal
	(and
	(preference p0A (or (available cdk2p1-cycA) (available pol)))
	(preference p0B (or (available DMP1-cycDp1) (available p107)))
	(preference p0C (or (available SP1-p107p1-gP) (available cdk2-cycA-E2F13)))
	(preference p0D (or (available cycEp1) (available p107-E2F4-DP12p1-gE2)))
	(preference p1A (not (num-subs l1)))
	(preference p2A (not (num-subs l2)))
	(preference p3A (not (num-subs l3)))))


(:metric minimize (+ (* 1.2 (is-violated p0A))
		     (* 2.0 (is-violated p0B))
		     (* 1.5 (is-violated p0C))
		     (* 2.0 (is-violated p0D))
		     (* 1 (is-violated p1A))
