...
(:init
	...
	(= (drive-cost market1 depot1) 496.00)
	(= (drive-cost depot1 market1) 496.00)
	(= (drive-time market1 depot1) 1488.00)
	(= (drive-time depot1 market1) 1488.00)
...
(:constraints (and
	...
	(preference p0A (always (>= (stored goods3) (stored goods2))))

	(forall (?t - truck)
	   (preference p4A (always-within 1604.00 (> (loaded goods3 ?t) 0)
			      (= (loaded goods3 ?t) 0))))

	(forall (?t - truck)
	   (preference p4B (always-within 1604.00 (> (loaded goods4 ?t) 0)
			      (= (loaded goods4 ?t) 0))))

	(forall (?t - truck)
	   (preference p3A (sometime (exists (?g - goods) (> (loaded ?g ?t) 0)))))

	(preference p0B (sometime-before (> (stored goods1) 0)
			   (>= (stored goods5) (request goods5))))))
